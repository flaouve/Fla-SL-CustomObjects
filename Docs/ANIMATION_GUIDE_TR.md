...
4. **Durumların (States) Animator'a Eklenmesi:**
   ...
   * **Çok Önemli:** Bu durumların (State) isimleri tam olarak `open` ve `close` olmalıdır (küçük harflerle).
...
3. Tetikleyiciyi seçin ve Inspector'dan şu ayarları yapın:
   ...
   * **`Animation State Name 2`**: İkinci etkileşimde (kapatmak için) oynatılacak durum adını yazın: **`close`**
...
2. Derleyici, animasyonlarınızı otomatik olarak sunucunun okuyabileceği bir **AssetBundle** dosyasına (`Cube` veya şablon isminize göre uzantısız dosya) paketleyecektir.
...
1. Sunucunuzda Remote Admin (RA) konsolunu açın.
2. Şablonunuzu spawn edin:
   `mp spawn test3`
3. Spawn edilen tetikleyicinin yanına gidin ve **E** tuşuna basın.
4. Kapınızın başarıyla açıldığını (`open`) ve bir kez daha bastığınızda kapandığını (`close`) göreceksiniz!
